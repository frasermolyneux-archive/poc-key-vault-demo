resource "azurerm_resource_group" "fd" {
  ...
  name     = format("rg-fd-%s-%s-%s", random_id.environment_id.hex, var.environment, each.value)
  location = each.value
...
resource "azurerm_cdn_frontdoor_profile" "fd" {
  ...
  resource_group_name = azurerm_resource_group.fd[var.primary_location].name
